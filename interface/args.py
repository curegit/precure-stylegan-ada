...
class CustomArgumentParser(ArgumentParser):
	...
	def add_output_args(self, default_dest):
		...
		group.add_argument("-f", "--force", action="store_true", help="allow overwriting existing files")
	...
	def add_generation_args(self, allow_zero=False):
		...
		group.add_argument("-s", "--similar", "--latent", "--center", metavar="LATENT_FILE", dest="center", help="shift the mean of random latent vectors to a specified one")
		group.add_argument("-d", "--deviation", "--sd", metavar="SIGMA", dest="sd", type=positive, default=1.0, help="the standard deviation of the latent vectors")
