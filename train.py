...
def parse_args():
	...
	parser.add_argument("dataset", metavar="DATASET_DIR", nargs="+", help=f"dataset directory containing real images (specify multiple directories to train conditional models, one directory per image class), supported file extensions: {', '.join(ImageDataset.extensions)}")
	parser.add_argument("-p", "--preload", action="store_true", help="preload the whole dataset into the RAM")
	parser.add_argument("-l", "--labels", metavar="CLASS", nargs="*", help="embed data class labels into output generators (e.g., cat, dog, ...) (specify as many CLASS as dataset directories), dataset directory names are used automatically if no CLASS arguments are given")
	...
	group.add_argument("-e", "--epoch", metavar="N", type=uint, default=1, help="training duration in epoch (note that elapsed training duration will not be serialized in snapshots)")
	...
	group.add_argument("-g", "--group", metavar="N", dest="group", type=uint, default=0, help="group size of the minibatch standard deviation (set to 0 to use entire batch)")
	...
	group.add_argument("-r", "--r1-gamma", metavar="GAMMA", dest="gamma", type=ufloat, default=10.0, help="coefficient of the R1 regularization (set to 0 to disable)")
	group.add_argument("-i", "--r1-interval", metavar="ITER", dest="r1", type=natural, default=16, help="apply the R1 regularization every ITER iteration (lazy regularization)")
	group.add_argument("-w", "--pl-weight", metavar="W", dest="weight", type=ufloat, default=2.0, help="coefficient of the path length regularization (set to 0 to disable)")
	...
	group.add_argument("-A", "--alpha", "--lr", metavar="ALPHA", type=positive, default=0.002, help="Adam's learning rate (used by both the generator and the discriminator)")
	...
	group.add_argument("-a", "--ada", action="store_true", help="enable Adaptive Discriminator Augmentation (ADA)")
	...
	group.add_argument("-D", "--delta", metavar="N", type=natural, default=500000, help="control the amount of an augmentation probability update (use a smaller value for larger updates)")
	...
	parser.add_argument("-Q", "--sleep", metavar=("N", "M"), nargs=2, type=uint, help="sleep N seconds every M iterations to slow down (intentional throttling)")
	...
	parser.add_argument("-n", "--number", metavar="N", type=uint, default=32, help="the number of intermediate images to be generated at each save time")
	return parser.add_evaluation_args(include_batch=False, include_noise=False, use_gpu_default=True).parse_args()
